    ...
    for episode in range(5):
        ...
        with torch.no_grad():
            ...
            #while not done:
             ...
             #   pred_next_obs, pred_rewards, pred_dones, model_state = model_env.step(
             #       actions, model_state, sample=False
             #   )
                pred_next_obs, pred_rewards, pred_dones,  _ = model_env.dynamics_model.sample_ours(actions, obs, deterministic=True)
                dones = termination_fn(actions, pred_next_obs)
                obs, reward, done = pred_next_obs.detach(), pred_rewards.detach(), dones.detach()
                avg_model_rew += reward
             #   print(reward, done)

    return avg_episode_reward / num_episodes, avg_model_rew / num_episodes
